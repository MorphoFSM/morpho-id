...
class SpecimenController extends Controller
{
    ...
    public function admin()
    {
        ...
        return view('specimenmanage', compact('specimens', 'kategori_list', 'induk_list'));
    }

    public function index()
    {
        $kategori_induk = Category::whereNull('parent_id')->with(['children', 'specimens'])->get();
        return view('public.index', compact('kategori_induk'));
    }
    ...
    public function store(Request $request)
  {
    ...
    public function explore(Request $request)
    {
        ...
        return view('public.explore', compact('specimens', 'senarai_tag', 'kategori_semasa'));
    }
    ...
    public function show($id)
    {
        ...
        return view('public.show_specimen', compact('item_semasa', 'sub_items', 'breadcrumbs'));
    }
...
public function compare(Request $request)
{
    ...
    return view('public.compare', compact('specimenA', 'specimenB', 'subA', 'subB'));
}
    ...
    public function edit($id)
    {
        ...
        $kategori_list = Category::whereNotNull('parent_id')->get();
        $induk_list = Category::whereNull('parent_id')->get();
        return view('edit_specimen', compact('specimen', 'kategori_list', 'induk_list'));
    }
    ...
    public function update(Request $request, $id)
{
    ...
    {
        \Log::info("Starting PDF generation for ID: $id");
        set_time_limit(0); // Prevent timeout when fetching large images
        $specimen = Specimen::with(['category', 'category.parent'])->findOrFail($id);
        ...
        if ($specimen->gambar && $specimen->gambar !== '-') {
            try {
                $response = Http::withoutVerifying()->timeout(15)->get($specimen->gambar);
                if ($response->successful()) {
                    $ext = pathinfo(parse_url($specimen->gambar, PHP_URL_PATH), PATHINFO_EXTENSION) ?: 'jpg';
                    $tmpPath = public_path('temp/main_' . uniqid() . '.' . $ext);
        ...
        $galleryData = [];
        if (is_array($specimen->galeri)) {
            foreach ($specimen->galeri as $galUrl) {
                if ($galUrl && $galUrl !== '-') {
                    try {
                        $response = Http::withoutVerifying()->timeout(15)->get($galUrl);
                        if ($response->successful()) {
                            $ext = pathinfo(parse_url($galUrl, PHP_URL_PATH), PATHINFO_EXTENSION) ?: 'jpg';
                            $tmpPath = public_path('temp/gal_' . uniqid() . '.' . $ext);
                            file_put_contents($tmpPath, $response->body());
                            $tempFiles[] = $tmpPath;
                            $galleryData[] = $tmpPath;
                            \Log::info("Successfully downloaded gallery image: $galUrl");
                        } else {
                            \Log::warning("Failed to download gallery image: $galUrl, Status: " . $response->status());
                        }
                    } catch (\Exception $e) {
                        \Log::error("Exception downloading gallery image: $galUrl, Error: " . $e->getMessage());
                    }
                }
            }
        }
        
        \Log::info("Total gallery images collected: " . count($galleryData));
        $pdf = Pdf::setOptions(['isRemoteEnabled' => true, 'chroot' => public_path()])->loadView('pdf.specimen', compact('specimen', 'imageData', 'galleryData'));
        
        // Generate PDF string and clean up temp files
        ...
        return response($pdfContent, 200, [
            ...
            'Content-Disposition' => 'attachment; filename="'.str_replace(' ', '_', $specimen->nama_spesimen).'_Report.pdf"'
        ]);
    }
    ...
    {
        set_time_limit(0); // Prevent timeout when fetching large images
        $specimen = Specimen::with('category')->findOrFail($id);
        
        $zip = new \ZipArchive();
        $zipFileName = 'Specimen_' . str_replace(' ', '_', $specimen->nama_spesimen) . '_' . time() . '.zip';
        ...
        if ($zip->open($zipPath, \ZipArchive::CREATE) === TRUE) {
            ...
            if ($specimen->gambar && $specimen->gambar !== '-') {
                try {
                    // Suppress warnings in case of network issue
                    $response = Http::withoutVerifying()->timeout(15)->get($specimen->gambar);
                    if ($response->successful()) {
                        $content = $response->body();
                        $ext = pathinfo(parse_url($specimen->gambar, PHP_URL_PATH), PATHINFO_EXTENSION) ?: 'jpg';
                        $zip->addFromString('main_image.' . $ext, $content);
                    }
                } catch (\Exception $e) {}
            }

            // Add gallery images
            if ($specimen->galeri && is_array($specimen->galeri)) {
                foreach ($specimen->galeri as $index => $url) {
                    if ($url) {
                        try {
                            ...
                            if ($response->successful()) {
                                ...
                                $zip->addFromString('gallery_' . ($index + 1) . '.' . $ext, $content);
                            }
                        } catch (\Exception $e) {}
                    }
                }
            ...
            // Add info text
            $info = "MORPHO.ID SPECIMEN INFO\n";
            $info .= "========================\n\n";
            $info .= "Name: " . $specimen->nama_spesimen . "\n";
            $info .= "Category: " . ($specimen->category ? $specimen->category->nama_kategori : 'None') . "\n";
            $tags = is_array($specimen->ciri_ciri) ? implode(', ', $specimen->ciri_ciri) : (is_string($specimen->ciri_ciri) ? $specimen->ciri_ciri : '');
            $info .= "Tags: " . $tags . "\n\n";
            $info .= "Description:\n" . strip_tags($specimen->penerangan) . "\n";
            
            $zip->addFromString('specimen_info.txt', $info);
            $zip->close();
        }
